...
from mysite.db.schema import UserProfileRegistrationShema, UserProfileLoginShema
...
async def register(user: UserProfileRegistrationShema, db: Session = Depends(get_db)):
    user_db = db.query(UserProfile).filter(UserProfile.username == user.username).first()
    if user_db:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="myndai adam registratia bolgon")
...
async def login(user: UserProfileLoginShema, db: Session = Depends(get_db)):
    user_db = db.query(UserProfile).filter(UserProfile.username == user.username).first()
    if not user_db or not verify_password(user.password, user_db.password):
        raise HTTPException(status_code=401, detail="username же password тура эмес")
...
async def logout(token: str, db: Session = Depends(get_db)):
    ...
    if not token_db:
        raise HTTPException(status_code=404, detail='Malymat tura emes')
#    user_token = db.query(UserProfile).filter(UserProfile.id == token_db.user_id).first()
    db.delete(token_db)
    db.commit()
    return {'message': 'Susses 200 ok'}
